...
p=pathlib.Path('Subtitle Projects/How to be Break Free From The Past, Be Happy and Find Your Greater Purpose'); w=p/'work'
events=json.loads((w/'youtube/9on5PnWPlk4.en-orig.json3').read_text())['events']; words=[]
...
for i,x in enumerate(words):
 ...
 group.append(x)
 nxt=words[i+1]['ms'] if i+1<len(words) else 6370920
...
m=import_source_job(w/'source.youtube.segmented.json',source_language='en',target_language='Simplified Chinese',output_root=w/'translation',title='Break Free From The Past');job=pathlib.Path(m['job_dir']);plan_job(job,max_cues=100,max_duration_sec=1200)
